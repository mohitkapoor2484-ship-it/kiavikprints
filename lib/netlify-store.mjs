...
function sanitizeProductInput(input, existing = null) {
  ...
  if (isClickerProduct(category, subcategory) && customTextEnabled && sizeOptions.some((size) => !parseClickerGridSize(size))) {
...
export async function upsertProduct(input) {
  ...
  if (id) {
    ...
    const updated = await getPool().query(
      `update kp_products set slug=$2,name=$3,category=$4,subcategory=$5,description=$6,price_cents=$7,image_data=$8,
       size_options=$9::jsonb,color_mode=$10,color_slot_count=$11,color_options=$12::jsonb,base_color_options=$13::jsonb,button_color_options=$14::jsonb,text_color_options=$15::jsonb,custom_text_enabled=$16,is_active=$17,updated_at=now()
  ...
  const created = await getPool().query(
    `insert into kp_products (id,slug,name,category,subcategory,description,price_cents,image_data,size_options,color_mode,color_slot_count,color_options,base_color_options,button_color_options,text_color_options,custom_text_enabled,is_active)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12::jsonb,$13::jsonb,$14::jsonb,$15::jsonb,$16,$17) returning *`,
    [
...
async function buildOrderInput(input, user, paymentMethod, paymentStatus) {
  ...
  for (const raw of items) {
    ...
    if (isClickerProduct(product.category, product.subcategory) && customText) {
      const grid = parseClickerGridSize(raw.sizeChoice);
      if (!grid) throw new Error(`${product.name} needs a clicker size in XxY format.`);
