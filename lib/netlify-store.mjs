...
export async function upsertProduct(input) {
  ...
  if (id) {
    ...
    const updated = await getPool().query(
      `update kp_products set slug=$2,name=$3,category=$4,description=$5,price_cents=$6,image_data=$7,
       size_options=$8::jsonb,color_mode=$9,color_slot_count=$10,color_options=$11::jsonb,base_color_options=$12::jsonb,button_color_options=$13::jsonb,text_color_options=$14::jsonb,custom_text_enabled=$15,is_active=$16,updated_at=now()
       where id=$1 returning *`,
      [
        id, p.slug, p.name, p.category, p.description, p.priceCents, p.imageData, JSON.stringify(p.sizeOptions),
        ...
        JSON.stringify(p.textColorOptions), p.customTextEnabled, p.isActive,
      ],
    );
  ...
  const created = await getPool().query(
    `insert into kp_products (id,slug,name,category,description,price_cents,image_data,size_options,color_mode,color_slot_count,color_options,base_color_options,button_color_options,text_color_options,custom_text_enabled,is_active)
     values ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10,$11::jsonb,$12::jsonb,$13::jsonb,$14::jsonb,$15,$16) returning *`,
    [
      makeId(), p.slug, p.name, p.category, p.description, p.priceCents, p.imageData, JSON.stringify(p.sizeOptions),
      p.colorMode, p.colorSlotCount, JSON.stringify(p.colorOptions), JSON.stringify(p.baseColorOptions), JSON.stringify(p.buttonColorOptions),
      JSON.stringify(p.textColorOptions), p.customTextEnabled, p.isActive,
    ],
  );
