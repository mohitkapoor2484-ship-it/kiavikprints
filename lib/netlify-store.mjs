...
function isChecked(value) {
  return value === true || ["1", "true", "on", "yes"].includes(normalizeString(value).toLowerCase());
}
...
function toUser(row) {
  if (!row) return null;
  const savedAddress = row.saved_address_line1 ? {
    line1: row.saved_address_line1,
    suburb: row.saved_suburb,
    state: row.saved_state,
    postcode: row.saved_postcode,
    country: row.saved_country || "Australia",
    label: [row.saved_address_line1, row.saved_suburb, row.saved_state, row.saved_postcode].filter(Boolean).join(", "),
  } : null;
  return {
    id: row.id,
...
function savedAddressFromInput(input) {
  const address = {
    line1: normalizeString(input.savedAddressLine1),
    suburb: normalizeString(input.savedSuburb),
    state: normalizeString(input.savedState),
    postcode: normalizeString(input.savedPostcode),
    country: normalizeString(input.savedCountry),
  };
  const hasAnyValue = Object.values(address).some(Boolean);
  if (!hasAnyValue) return address;
  if (!isChecked(input.savedAddressVerified)) throw new Error("Choose a saved address from the address search results.");
  if (!address.line1 || !address.suburb || !address.state || !address.postcode || address.country !== "Australia") {
    throw new Error("Choose a complete Australian address from the address search results.");
  }
  return address;
}
...
export async function getSessionByToken(token) {
  ...
  const { rows } = await getPool().query(
    `select s.id,s.user_id,s.token,s.expires_at,u.full_name,u.email,u.is_admin,u.created_at as user_created_at,
            u.saved_address_line1,u.saved_suburb,u.saved_state,u.saved_postcode,u.saved_country
  ...
  return {
    ...
    user: {
      ...
      savedAddress: row.saved_address_line1 ? {
        line1: row.saved_address_line1,
        suburb: row.saved_suburb,
        state: row.saved_state,
        postcode: row.saved_postcode,
...
export async function signupUser(input) {
  ...
  const savedAddress = savedAddressFromInput(input);
  ...
  const { rows } = await getPool().query(
    `insert into kp_users (id,full_name,email,password_hash,is_admin,saved_address_line1,saved_suburb,saved_state,saved_postcode,saved_country)
     values ($1,$2,$3,$4,false,$5,$6,$7,$8,$9) returning *`,
    [makeId(), fullName, email, hashPassword(password), savedAddress.line1, savedAddress.suburb, savedAddress.state, savedAddress.postcode, savedAddress.country],
  );
...
function sanitizeProductInput(input, existing = null) {
  ...
  const rawStockQuantity = input.stockQuantity !== undefined ? input.stockQuantity : existing?.stock_quantity;
  const stockQuantity = rawStockQuantity === null || normalizeString(rawStockQuantity) === "" ? null : Number(rawStockQuantity);
  if (stockQuantity !== null && (!Number.isInteger(stockQuantity) || stockQuantity < 0)) throw new Error("Stock quantity must be a whole number of zero or more.");
  ...
  return {
    ...
    isComingSoon: input.isComingSoon === undefined ? Boolean(existing?.is_coming_soon) : Boolean(input.isComingSoon),
  };
}
...
export async function upsertProduct(input) {
  ...
  if (id) {
    ...
    const updated = await getPool().query(
      `update kp_products set slug=$2,name=$3,category=$4,subcategory=$5,description=$6,price_cents=$7,image_data=$8,
       size_options=$9::jsonb,color_mode=$10,color_slot_count=$11,color_options=$12::jsonb,base_color_options=$13::jsonb,button_color_options=$14::jsonb,text_color_options=$15::jsonb,custom_text_enabled=$16,is_active=$17,extra_clicker_price_cents=$18,extra_text_clicker_price_cents=$19,is_coming_soon=$20,stock_quantity=$21,updated_at=now()
      ...
      [
        ...
        JSON.stringify(p.textColorOptions), p.customTextEnabled, p.isActive, p.extraClickerPriceCents, p.extraTextClickerPriceCents, p.isComingSoon, p.stockQuantity,
      ],
    );
  ...
  const created = await getPool().query(
    `insert into kp_products (id,slug,name,category,subcategory,description,price_cents,image_data,size_options,color_mode,color_slot_count,color_options,base_color_options,button_color_options,text_color_options,custom_text_enabled,is_active,extra_clicker_price_cents,extra_text_clicker_price_cents,is_coming_soon,stock_quantity)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12::jsonb,$13::jsonb,$14::jsonb,$15::jsonb,$16,$17,$18,$19,$20,$21) returning *`,
    [
...
async function buildOrderInput(input, user, paymentMethod, paymentStatus) {
  ...
  for (const raw of items) {
    ...
    if (Boolean(product.is_coming_soon)) throw new Error(`${product.name} is coming soon and cannot be ordered yet.`);
  ...
  if (paymentStatus !== "draft" && !isChecked(input.orderConfirmed)) {
    throw new Error("Please confirm that your items and contact details are correct before placing the order.");
  }
  ...
  if (deliveryMethod !== "pickup") {
    if (!isChecked(input.addressVerified)) throw new Error("Select a delivery address from the address search results.");
...
async function persistOrder(order) {
  ...
  try {
    ...
    await insertOrderItems(client, order);
    if (shouldReduceStock(order.paymentStatus)) await reduceOrderStock(client, order.items);
    await client.query("COMMIT");
...
async function insertOrderItems(client, order) {
...
function shouldReduceStock(paymentStatus) {
  return paymentStatus === "awaiting_cash_pickup" || paymentStatus === "paid";
}

async function reduceOrderStock(client, items) {
  const requested = new Map();
  for (const item of items) {
    const current = requested.get(item.productId) || { quantity: 0, name: item.productName };
    current.quantity += Number(item.quantity || 0);
    requested.set(item.productId, current);
  }
  for (const [productId, request] of [...requested.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    const result = await client.query(
      `update kp_products set stock_quantity=stock_quantity-$2,updated_at=now()
       where id=$1 and (stock_quantity is null or stock_quantity >= $2) returning stock_quantity`,
      [productId, request.quantity],
    );
    if (!result.rows[0]) throw new Error(`${request.name} no longer has enough stock available.`);
  }
}
...
function orderItemInput(item) {
  return {
    ...
    textColorChoice: item.textColorChoice,
  };
}

function mergeDraftItems(items) {
  const merged = new Map();
  for (const item of items) {
    const normalized = { ...item, quantity: Math.max(1, Math.min(99, Math.floor(normalizeNumber(item.quantity, 1)))) };
    const key = JSON.stringify([normalized.productId, normalized.sizeChoice || "", normalized.colorChoices || [normalized.colorChoice || ""], normalized.customText || "", normalized.textColorChoice || ""]);
    const existing = merged.get(key);
    if (existing) existing.quantity = Math.min(99, existing.quantity + normalized.quantity);
    else merged.set(key, normalized);
  }
  return [...merged.values()];
}

async function findUserDraft(userId) {
  const { rows } = await getPool().query(
    "select * from kp_orders where user_id=$1 and payment_status='draft' order by updated_at desc limit 1",
    [userId],
  );
  return rows[0] || null;
}

async function replaceUserDraftOrder(draftId, userId, order) {
  const client = await getPool().connect();
  const savedOrder = { ...order, id: draftId };
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
      `update kp_orders set customer_name=$3,customer_email=$4,customer_phone=$5,shipping_name=$6,shipping_email=$7,shipping_phone=$8,
       address_line1=$9,address_line2=$10,suburb=$11,state=$12,postcode=$13,country=$14,delivery_method=$15,order_notes=$16,
       subtotal_cents=$17,shipping_cents=$18,total_cents=$19,currency_code=$20,payment_method=$21,payment_status=$22,
       fulfillment_status=$23,paypal_order_id='',paypal_capture_id='',updated_at=now()
       where id=$1 and user_id=$2 and payment_status='draft' returning *`,
      [
        draftId, userId, savedOrder.customerName, savedOrder.customerEmail, savedOrder.customerPhone, savedOrder.shippingName,
        savedOrder.shippingEmail, savedOrder.shippingPhone, savedOrder.addressLine1, savedOrder.addressLine2, savedOrder.suburb,
        savedOrder.state, savedOrder.postcode, savedOrder.country, savedOrder.deliveryMethod, savedOrder.orderNotes,
        savedOrder.subtotalCents, savedOrder.shippingCents, savedOrder.totalCents, savedOrder.currencyCode, savedOrder.paymentMethod,
        savedOrder.paymentStatus, savedOrder.fulfillmentStatus || "received",
      ],
    );
    if (!rows[0]) throw new Error("Draft order not found.");
    await client.query("delete from kp_order_items where order_id=$1", [draftId]);
    await insertOrderItems(client, savedOrder);
    if (shouldReduceStock(savedOrder.paymentStatus)) await reduceOrderStock(client, savedOrder.items);
    await client.query("COMMIT");
    return toOrder(rows[0], savedOrder.items.map((item) => toOrderItem({
      id:item.id, product_id:item.productId, product_name:item.productName, unit_price_cents:item.unitPriceCents, quantity:item.quantity,
      size_choice:item.sizeChoice, color_choice:item.colorChoice, color_choices:item.colorChoices, custom_text:item.customText,
      text_color_choice:item.textColorChoice, line_total_cents:item.lineTotalCents,
    })));
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
...
async function requireSelectedDraft(input, user) {
  const draftId = normalizeString(input.draftOrderId);
  if (!draftId) return null;
  if (!user?.id) throw new Error("Sign in to place a saved draft order.");
  const draft = await findUserDraft(user.id);
  if (!draft || draft.id !== draftId) throw new Error("The selected draft order is no longer available.");
  return draft;
}

export async function createPreviewOrder(input, user = null) {
  await ensureDefaults();
  if (!user?.id) throw new Error("Sign in to save and edit a draft order.");
  const selected = await requireSelectedDraft(input, user);
  const existing = selected || await findUserDraft(user.id);
  const draftInput = selected
    ? input
    : existing
      ? { ...input, items: mergeDraftItems([...(await getOrderItems(existing.id)).map(orderItemInput), ...(Array.isArray(input.items) ? input.items : [])]) }
      : input;
  const order = { ...(await buildOrderInput(draftInput, user, "draft", "draft")), fulfillmentStatus: "draft" };
  return existing ? replaceUserDraftOrder(existing.id, user.id, order) : persistOrder(order);
}

export async function createDraftOrder(input, user = null) {
  await ensureDefaults();
  const selected = await requireSelectedDraft(input, user);
  const order = { ...(await buildOrderInput(input, user, "paypal", "pending")), fulfillmentStatus: "awaiting_payment" };
  return selected ? replaceUserDraftOrder(selected.id, user.id, order) : persistOrder(order);
}
...
export async function updateOrderPayment(orderId, patch) {
  ...
  try {
    await client.query("BEGIN");
    const current = await client.query("select * from kp_orders where id=$1 for update", [orderId]);
    if (!current.rows[0]) throw new Error("Order not found.");
    const nextStatus = patch.paymentStatus ?? current.rows[0].payment_status;
    const items = await getOrderItems(orderId, client);
    if (shouldReduceStock(nextStatus) && !shouldReduceStock(current.rows[0].payment_status)) {
      await reduceOrderStock(client, items);
    }
    const { rows } = await client.query(
